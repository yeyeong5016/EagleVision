...
class SHIPRSImageNETDataset(BaseDataset):
    ...
    METAINFO = {
        'classes':
        ('Container Ship', 'Enterprise', 'Tugboat', 'Cargo', 'Sanantonio AS', 'YuDeng LL', 'Test Ship', 'Ferry', 'Perry FF', 'Yacht', 'Other Aircraft Carrier', 'Atago DD', 'Austin LL', 'Other Destroyer', 'Other Ship', 'EPF', 'Sailboat', 'YuDao LL', 'Asagiri DD', 'Medical Ship', 'Oil Tanker', 'Fishing Vessel', 'Patrol', 'Midway', 'Other Merchant', 'Hovercraft', 'Commander', 'Hatsuyuki DD', 'Masyuu AS', 'Other Auxiliary Ship', 'Nimitz', 'LHA LL', 'Motorboat', 'Other Warship', 'Hyuga DD', 'YuTing LL', 'Arleigh Burke DD', 'LSD 41 LL', 'Barge', 'Submarine', 'Training Ship', 'Wasp LL', 'Osumi LL', 'Ticonderoga', 'AOE', 'RoRo', 'Other Landing', 'Dock', 'YuZhao LL', 'Other Frigate'),
    ...
    def find_subsequence_range(self, tensor, subsequence):
        for i in range(len(tensor) - len(subsequence) + 1):
            if torch.equal(tensor[i:i+len(subsequence)], subsequence):
                return range(i, i + len(subsequence))  # 直接返回 range
                    ...
                    with open(txt_file) as f:
                        ...
                        for si in s:
                            ...
                            for i, (k, v) in enumerate(info_dict.items()):
                                ...
                                attr_token.append(self.tokenizer.encode(str(v)))
    ...
    def get_cat_ids(self, idx: int) -> List[int]:
        """Get SHIPRSImageNet category ids by index.
