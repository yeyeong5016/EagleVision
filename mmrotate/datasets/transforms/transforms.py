...
class EVPackDetInputs(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for key in self.mapping_table.keys():
            ...
            elif key == 'gt_captions':
                if 'gt_ignore_flags' in results:
                    instance_data[
                        self.mapping_table[key]] = np.array(results[key])[valid_idx]
                    ignore_instance_data[
                        self.mapping_table[key]] = np.array(results[key])[ignore_idx]
                else:
                    instance_data[self.mapping_table[key]] = np.array(results[key])
            else:
                if 'gt_ignore_flags' in results:
                    ...
                    instance_data[self.mapping_table[key]] = to_tensor(
                        results[key])
        if "gt_caption_ignore_flags" in results:
            instance_data["gt_caption_ignore_flags"] = results["gt_caption_ignore_flags"][valid_idx]
            ignore_instance_data["gt_caption_ignore_flags"] = results["gt_caption_ignore_flags"][ignore_idx]
